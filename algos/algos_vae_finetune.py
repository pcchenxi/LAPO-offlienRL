...
class Latent(object):
    ...
    def train(self, iterations, batch_size=100):
        for it in range(iterations):
            ...
            action_explore = action + (torch.randn_like(action) * 0.1).clamp(-0.3, 0.3)
            ...
            q_a1, q_a2 = self.critic_target(state, action_explore)
            ...
            # train weighted CVAE
            recons_action, z_sample, mu, log_var = self.actor_vae(state, action_explore)

            recons_loss_ori = F.mse_loss(recons_action, action_explore, reduction='none')
            ...
            if not self.no_piz:
                ...
                actor_action = actor_action_ori + (torch.randn_like(actor_action_ori) * 0.02).clamp(-0.05, 0.05)
