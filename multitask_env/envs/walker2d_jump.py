...
class Walker2dJumpEnv(mujoco_env.MujocoEnv, utils.EzPickle):
    ...
    def step(self, a):
        ...
        # reward = alive_bonus + reward_jump + reward_ctrl
        reward = alive_bonus + reward_jump + reward_ctrl - np.abs(reward_run)
        # done = not (height > 0.8 and height < 2.0 and
        #             ang > -1.0 and ang < 1.0)
        done = not (height > 0.8 and
                    ang > -1.0 and ang < 1.0)
        ob = self._get_obs()
    ...
    def get_normalized_score(self, score):
        min_score = 0.71875006 * 1000
        max_score = 2.2779615 * 1000

        return (score - min_score)/(max_score-min_score)

    def get_dataset(self):
        home = str(Path.home())
        with open(home + "/.d4rl/datasets/multi-walker2d-jump-v1.pkl", 'rb') as handle:
